...
//            따라서 realEnd가 0이라면 endPage를 1로 변경해주어야 한다.
    ...
    public ProductPageDTO reviewProductPageDTO(ProductCriteria productCriteria, int total){
        log.info("reviewProductPageDTO: " + productCriteria);
        return reviewProductPageDTO(productCriteria, total, 5);
    }

    public ProductPageDTO reviewProductPageDTO(ProductCriteria productCriteria, int total, int pageCount){
        this.productCriteria = productCriteria;
        this.total = total;
        this.pageCount = pageCount;
//        현재 페이지를 기준으로 페이지 단위에 맞춰서 마지막 페이지 계산
