...
public class PartnerPageDTO {
    ...
    public PartnerPageDTO createPageDTO(PartnerCriteria partnerCriteria, int total) {
        return createPageDTO(partnerCriteria, total, 10);
    }

    public PartnerPageDTO createPageDTO(PartnerCriteria partnerCriteria, int total, int pageCount) {
        this.partnerCriteria = partnerCriteria;
        this.total = total;
        this.pageCount = pageCount;
//        현재 페이지를 기준으로 페이지 단위에 맞춰서 마지막 페이지 계산
        this.endPage = (int) (Math.ceil(partnerCriteria.getPage() / (double) pageCount)) * pageCount;
        this.startPage = endPage - pageCount + 1;
//        게시글 전체 개수를 통해 가장 마지막 페이지 계산
        this.realEnd = (int) (Math.ceil((double) total / partnerCriteria.getAmount()));
